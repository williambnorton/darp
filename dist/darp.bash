...
if [ $wireguard_rc -eq 0 -a $docker_rc -eq 0 ]; then
    ...
    do
        ...
        ( sleep 60;  docker save williambnorton/darp:latest | gzip -c > ~/wireguard/darpdocker.tgz; echo `date`" DOCKER Cached-could be named with version"; echo `date` "cached DARP" `curl localhost:65013/version|awk -F. '{ print "Docker."$2"."$3 }'| awk -F: '{ print $1}'` ) & #cache docker
