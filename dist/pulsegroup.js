...
var AugmentedPulseGroup = /** @class */ (function () {
    function AugmentedPulseGroup(config, pulseGroup) {
                ...
                if (pulseEntry.owl == NO_MEASURE) {
                    ...
                    if (pulseEntry.medianHistory.length > 0 && pulseEntry.owl != 0) { //medianHistory will take a minute to get an entry
                        ...
                        if ((delta > 5) && (pulseEntry.owl > 3) && (deviation > 20)) { //flag if off by 30% from median
                            ...
                            console.log("pulse(): Flagging " + pulseEntry.mint + "-" + _this.mintTable[0].mint + "=" + pulseEntry.owl + "  delta=" + delta + " geo=" + pulseEntry.geo + " --> " + _this.config.GEO + " nodeEntry.owl=" + pulseEntry.owl + "@ medianOfMeasures=" + medianOfMeasures + " medianOfMedians=" + medianOfMedians + " deviation=" + deviation + "%");
        ...
        this.owls = owls.replace(/,+$/, ""); // remove trailing comma
...
logger_1.logger.debug("pulse(): looking for my entry to pulse: " + this.config.GEO + ":" + this.groupName);
if (myEntry == null) {
    logger_1.logger.warning("Cannot find " + this.config.GEO + ":" + this.groupName);
}
else {
    myEntry.seq++;
...
this.mintTable[0].lastPulseTimestamp = lib_1.now();
// INSTRUMENTATION POINT shows load on node - DO NOT DELETE
//console.log(ts()+`** pulsing took=${now()%1000} ms`);
setTimeout(this.pulse, PULSEFREQ * 1000 - (lib_1.now() % 1000)); //pull back to second boundaries
;
isGenesisNode = function () {
...
//  @wbnwbnwbnwbn
//
getOWLfrom = function (srcMint, owls) {
    var ary = owls.split(",");
...
//  recvPulses
pulselayer_1.recvPulses = function () {
    var self = _this;
