...
//
//  mintList() - callback with list of mints/owls for this node - return comma separated list
//
      ...
      //console.log("insideIterator");
        if (err) {
          ...
          if ((SR) && (SR.owls() && (SR.owls != "" )) {
                callback(null,SR.owls.replace(/=-?[0-9]*/g,'')); //.split(",").sort().reverse(); //could sort here
          } else {
...
module.exports = { now: now, ts: ts, dump: dump, Log: Log, checkNoiaSWHash: checkNoiaSWHash, mintList: mintList, SRList:SRList, getMints, getOwls, YYMMDD, oneTimePulse, MYIP, MYVERSION  };
