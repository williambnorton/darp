...
function SRList(redisClient,callback) {  
    ...
    redisClient.hgetall("gSRlist", function(err, gSRlist) {
      if (err) {
        ...
        console.log("iterating SRlist(): gSRlist.length="+gSRlist.length+" gSRlist="+dump(gSRlist));
        if (gSRlist) {
          mySRList.push(gSRlist);
        } else {
            console.log("******************* Callback Time : "+dump(mySRList));
            callback(null,mySRList); //send back a list of mints
        }
...
module.exports = { now: now, ts: ts, dump: dump, Log: Log, checkNoiaSWHash: checkNoiaSWHash, mintList: mintList, SRList:SRList  };
