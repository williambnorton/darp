...
function fetchMint(mint) {
const http = require("http");
redisClient.hgetall("mint:0",function (err,me) {

    const url = "http://"+me.genesisIP+":"+me.genesisPort+"/mint/"+mint;
    console.log("fetchMint(): url="+url);
    http.get(url, res => {
        ...
        res.on("end", () => {
            ...
            console.log("mint"+mint+"="+body);
            redisClient.hmset("mint:"+mint,JSON.parse(body));
            console.log("mint"+mint+"="+body+" WRITTEN TO REDIS");
...
function mintList(redisClient,SR,callback) {
    ...
    redisClient.hgetall(SR, function(err, SR) {
      ...
      if (err) {
        ...
        if (SR  && SR.owls ) {
          owlList=SR.owls.replace(/=[0-9]*/g,'').split(",");
        } 
...
module.exports = { now: now, ts: ts, dump: dump, Log: Log, checkNoiaSWHash: checkNoiaSWHash, mintList: mintList, SRList:SRList, getMints, getOwls, fetchMint  };
