...
function getMints(redisClient,SR,callback) {
    redisClient.hgetall(SR, function(err, SR) {
        //console.log("insideIterator");
          if (err) {
                console.log("forEachMint(): hgetall SR mints "+SR+" failed");
          } else {
            if ((SR) && (SR.owls) && (SR.owls != "" )) {
              var mints=SR.owls.replace(/=-?[0-9]*/g,'');//.split(",").sort().reverse(); //could sort here
                callback(mints.split(","));  //return array
            }else {
                console.log("lib.js: ERROR forEachMint() called with SR==null");
                process.exit(86);
            }
        }
    });
...
//
//  forEachMint() - callback with list of mints/owls for this node - return comma separated list
//
function forEachMint(redisClient,SR,callback) {
    //fetch the group mints mint:2 : 2  mint:5 : 5   ....
    getMints(redisClient, SR, function (mints) {
        for(var mint in mints.split(","))
            callback(mint); 
    });
}
...
module.exports = { now: now, ts: ts, dump: dump, Log: Log, checkNoiaSWHash: checkNoiaSWHash, forEachMint, SRList:SRList, getMints, getOwls, YYMMDD, oneTimePulse, MYIP, MYVERSION  };
