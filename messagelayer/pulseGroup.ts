        ...
        for (var src in pulseGroup.matrix) {      
            ...
            if (mintEntry!=null) {
                ...
                for (var dest in pulseGroup.matrix[src]) {
                    ...
                    if (destMintEntry!=null) txt += '<td class="'+src+"-"+dest+' '+destMintEntry.geo+'">' + '<a target="_blank" href="http://' + destMintEntry.ipaddr + ':' + destMintEntry.port + '/graph/' + mintEntry.geo + '/' + destMintEntry.geo +'" >' + pulseGroup.matrix[src][dest] + " ms</a></td>";
