...
function fetchConfig(gSRlist, config, callback) {
    if (typeof config == "undefined" || config == null) {
        console.log("fetchConfig(): gSRlist=" + lib_1.dump(gSRlist) + " config=" + lib_1.dump(config) + " ");
        config = {
            gSRlist: gSRlist,
            mintTable: {},
            pulses: {},
    ...
    if (entry) {
        ...
        expressRedisClient.hgetall("mint:" + mint, function (err, mintEntry) {
            ...
            expressRedisClient.hgetall(entryLabel, function (err, pulseEntry) {
                console.log("EXPRESS() entryLabel=" + entryLabel + " pulseEntry=" + lib_1.dump(pulseEntry) + " config=" + lib_1.dump(config) + " gSRlist=" + lib_1.dump(gSRlist));
