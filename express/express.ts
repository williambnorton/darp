      ...
      //display(function (html) {
         makeConfigAll(function(config) {
            ...
            for (var a in pulses) {
               ...
               console.log(ts()+"pulseEntry="+pulseEntry+" pulseTable[pulseEntry]"+dump(pulses[a]));
...
function makeConfig(callback) {
   expressRedisClient.hgetall("mint:0", function(err, me) {
      expressRedisClient.hgetall("gSRlist", function(err,gSRlist) {
         console.log(process.env.VERSION+" gSRlist="+dump(gSRlist));
...
function fetchConfigAll(gSRlist, config, callback) {
   ...
   if (entry) {
      ...
      expressRedisClient.hgetall("mint:"+mint, function (err,mintEntry) {   
         ...
         if (mintEntry) config.mintTable["mint:"+mintEntry.mint] = mintEntry;  //set the pulseEntries
