...
function checkAdminControl() {
  ...
  redisClient.hget("mint:0", "adminControl", function(err,adminControl) {
    if (adminControl=="PULSE" && state!="PULSING") {
      console.log(ts()+"PULSING adminControl="+adminControl);
      pulse(1);
//      state="IDLE"
      ...
      res.on("end", () => {
        ...
        if (mintEntry==null || typeof mintEntry.geo=="undefined") {
          ...
          redisClient.hmset("mint:"+mint, mintEntry, function (err,reply) {
            ...
            console.log("fetching mint from genesis node gave us: newSegmentEntry="+dump(newSegmentEntry));
   ...
   redisClient.hgetall("mint:"+mint, function (err, mintEntry) {
      ...
      if (mintEntry!=null) {
        ...
        if (mint!=null) {
          ...
          if (incomingMintList!="") buildPulsePkt(incomingMintList,pulseMsg,sendToAry);
          else {
            //
            //      message ready - pulse
