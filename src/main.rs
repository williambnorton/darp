...
const HELP_MSG: &str = "To build and run: ./rustping [optional args] ADDRESS
...
fn main() {
    ...
    while i < args.len() {
        match args[i].as_str() {
            "--ttl" => {
                i += 1;
                ttl = args[i].parse::<u8>()
                    .expect(format!("{} is not a valid TTL value! rustping unhappy :( ", args[i]).as_str());
            },
            "--timeout" => {
                i += 1;
                timeout_ms = args[i].parse::<u64>()
                    .expect(format!("{} is not a valid timeout value in ms! rustping unhappy :( ", args[i]).as_str());
            },
            "--delay" => {
                i += 1;
            ...
            "--iter" => {
                ...
                iterations = args[i].parse::<usize>()
                    .expect(format!("{} is not a valid number of iterations! rustping unhappy :( ", args[i]).as_str());
            },
            "-4" => {
                use_ipv4 = true;
            },
            ...
            _ => {
                if pos_args > pos_args_max {
                    eprintln!("Too many argument! rustping unhappy :(");
                    println!("{}", HELP_MSG);
                    std::process::exit(0);
                }else{
                    addr = &args[i];
    ...
    if pos_args != pos_args_max {
        eprintln!("You are missing an argument! rustping unhappy :(");
        println!("{}", HELP_MSG);
        std::process::exit(0);
    }

    if use_ipv4 && use_ipv6 {
        panic!("No, you can't use both IPv4 and IPv6 at the same time! rustping unhappy :(");
    }
...
fn ping(addr: &String, use_ipv4: bool, use_ipv6: bool, iterations: usize, ttl: u8, timeout_ms: u64, delay_ms: u64) {
    ...
    let (ip_addr, addr_name) = match net::IpAddr::from_str(addr) {
        Ok(ip) => {
            ...
            println!("Pinging {} ({}) with rustping's special abilities.", addr_name, ip);
            (ip, addr_name)
        },
        Err(_) => {
            // at this point, ip_addr must be a hostname or an invalid IPv4/IPv6 address
            let ips = dns_lookup::lookup_host(addr.as_str())
                .expect(format!("Could not perform DNS lookup on {}! rustping unhappy :( ", addr).as_str());

            let ip = if use_ipv4 {
                ips.into_iter().find(|&ip| ip.is_ipv4())
                    .expect(format!("No IPv4 address found for {}! rustping unhappy :( ", addr).as_str())
            }else if use_ipv6 {
                ips.into_iter().find(|&ip| ip.is_ipv6())
                    .expect(format!("No IPv6 address found for {}! rustping unhappy :( ", addr).as_str())
            }else{
                // just get any address, if possible
                *ips.get(0).expect(format!("No IP address found for {}! rustping unhappy :( ", addr).as_str())
            };

            println!("Pinging {} ({}) with rustping's special abilities.", addr, ip);
    ...
    let (mut sender, receiver) = match ip_addr {
        net::IpAddr::V4(_) => {
            ...
            transport_channel(1024, TransportChannelType::Layer4(
                ...
                .expect("Unable to open transport channel! rustping unhappy :( ")
        },
        net::IpAddr::V6(_) => {
            transport_channel(1024, TransportChannelType::Layer4(
                ...
                *total_packets_sent += 1;
                ...
                match ip_addr {
                    net::IpAddr::V4(_) => {
                        ...
                        sender.send_to(packet, ip_addr).expect("Error in sending packet! rustping unhappy :( ");
                    },
                    net::IpAddr::V6(ip) => {
    ...
    println!("\nDone! rustping sent {} packets total to {}.\n\trustping received {} ({:.1}%), lost {} ({:.1}%), and realized that {} ({:.1}%) timed out.",
    ...
    thread::spawn(move || {
        ...
        while not_done.load(atomic::Ordering::SeqCst) {
            ...
            let next_res = receiver_iter.next_with_timeout(receiver_delay)
                .expect("Error receiving packet! rustping unhappy :( ");
                ...
                icmp::IcmpTypes::EchoReply => {
                    ...
                    if res_identifier == identifier && (res_seq_num as usize) <= total_packets_sent {
                        ...
                        if received.contains(&res_seq_num) {
                            println!("rustping received a duplicate packet (seq num: {}) from {}!", res_seq_num, addr);
                        }else{
                            ...
                            if elapsed_ms > timeout.as_millis() {
                                timed_out_packets += 1;
                                println!("rustping received timed out packet (seq num: {}) from {} in {} ms!\n\tSent {}, with {} ({:.1}%) lost so far.",
                                ...
                                received_packets += 1;
                                println!("rustping received packet (seq num: {}) from {} in {} ms (avg rtt: {:.1} ms)!\n\tSent {}, with {} ({:.1}%) lost so far.",
                ...
                icmp::IcmpTypes::DestinationUnreachable => {
                    ...
                    if res_identifier == identifier && (res_seq_num as usize) <= total_packets_sent {
                        if received.contains(&res_seq_num) {
                            ...
                            lost_packets += 1;
                            println!("rustping received a destination unreachable packet (seq num: {}) from {} (code: {})!\n\tSent {}, with {} ({:.1}%) lost so far.",
                ...
                icmp::IcmpTypes::TimeExceeded => {
                    ...
                    if res_identifier == identifier && (res_seq_num as usize) <= total_packets_sent {
                        if received.contains(&res_seq_num) {
                            ...
                            lost_packets += 1;
                            println!("rustping received a time exceeded packet (seq num: {}) before reaching {} (last host: {}, code: {})!\n\tSent {}, with {} ({:.1}%) lost so far.",
                ...
                icmpv6::Icmpv6Types::EchoReply => {
                    ...
                    if res_identifier == identifier && (res_seq_num as usize) <= total_packets_sent {
                        ...
                        if received.contains(&res_seq_num) {
                            ...
                            if elapsed_ms > timeout.as_millis() {
                                timed_out_packets += 1;
                                println!("rustping received timed out packet (seq num: {}) from {} in {} ms (avg rtt: {:.1} ms)!\n\tSent {}, with {} ({:.1}%) lost so far.",
                                res_seq_num, addr, elapsed_ms, total_rtt as f64 / received_packets as f64, total_packets_sent, lost_packets, lost_packets as f64 / total_packets_sent as f64 * 100f64);
                            }else{
                                received_packets += 1;
                                println!("rustping received packet (seq num: {}) from {} in {} ms (avg rtt: {:.1} ms)!\n\tSent {}, with {} ({:.1}%) lost so far.",
